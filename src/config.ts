...
export class ConfigManager {
    ...
    public loadConfig(): Config {
        try {
            ...
            throw new ConfigError(`Failed to load configuration: ${errorMessage}`, error);
        }
    }
    ...
    public saveConfig(config: Config): void {
        try {
            ...
            const configData: string = JSON.stringify(normalizedConfig, null, 2);
            ...
            throw new ConfigError(`Failed to save configuration: ${errorMessage}`, error);
        }
    }
    ...
    public removeFilePath(filePath: string): boolean {
        ...
        config.filePaths = config.filePaths.filter((p: string) => p !== normalizedPath);
    ...
    public validateCurrentConfig(): void {
        ...
        if (invalidPaths.length > 0) {
            logger.warn(`Found ${invalidPaths.length} invalid file paths`, invalidPaths);
        }
    }
    ...
    private loadFromFile(): Config {
        try {
            ...
            const config: Config = {
                ...
                includeHeaders: parsedConfig.includeHeaders !== undefined ? parsedConfig.includeHeaders : true
            };
    ...
    private loadFromEnvironment(): Config {
        ...
        if (filePathsString) {
            config.filePaths = filePathsString
                .split(',')
                .map((filePath: string) => this.fileManager.normalizePath(filePath.trim()))
                .filter((filePath: string) => filePath.length > 0);

            if (config.filePaths.length > 0) {
                this.saveConfig(config);
                logger.info(`Configuration migrated from environment to ${this.configFile}`);
            }
        }
    ...
    private normalizeConfig(config: Config): Config {
        return {
            filePaths: config.filePaths.map((filePath: string) => this.fileManager.normalizePath(filePath)),
